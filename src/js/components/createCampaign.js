...
class RegisterUser extends Component {
    ...
    render() {
        ...
        return ( 
            ...
            <div className = "inputField" >
            <label > Total Spend <ToolTipMarker id = { _.uniqueId() }
            tooltip = "Total amount you are willing to spend." >
            <span > <Icon icon = "question-circle" / > </span> </ToolTipMarker> </label> <span className = { `inputContainer selectInput ${errors.budget?'error':''}` } >
            <input type = "text"
            name = "campaign_totalSpend"
            ...
            <select onChange = {
                (e) => this.setState({ budget_type: e.target.value }) }
            value = { budget_type } >
            <option value = "Daily" > Daily </option> <option value = "Lifetime" > Lifetime </option>

            </select> </span> </span> <span className = "minimumValues" > Minimum Amount is & #8358;12,000</span>
                  </div>
                  <div className= "inputField" >
            <label > Desired Period </label> {
                ...
                >
                <span className = { `radioLabel ${(budget_type==='Daily' && (dateFrom== moment().format('L')|| dateFrom==''))?'active':''}` } > Start Immediately </span> </span> <span className = { `inputContainer rangeInput ${errors.dateFrom?'error':''}` } >
                    <span className = "subLabel" > From </span> <DayPickerInput
                placeholder = "MM/DD/YYYY"
                onDayChange = {
                    (date) => { this.setState({ dateFrom: date }) } }
                ...
                onDayChange = {
                    (date) => { this.setState({ dateFrom: date }) } }
                dayPickerProps = { dayPickerPropsFrom }
                /> </span> <span className = "arrowRange rangeSeperator" > <Icon icon = "long-arrow-right" > </Icon></span >
                    <span className = { `inputContainer rangeInput ${errors.dateTo?'error':''}` } >
                    <span className = "subLabel" > To </span> <DayPickerInput
                placeholder = "MM/DD/YYYY"
                onDayChange = {
                    (date) => { this.setState({ dateTo: date }) } }
                dayPickerProps = { dayPickerPropsTo }
                /> </span> </span>
            } </div> </Col> {
                loading ? <Icon icon = "circle-o-notch fa-spin loading" / > :
                    <button className = "primaryButton"
                onClick = { this.createCampaign.bind(this) } > Create campaign </button>
            } {
                createSuccess ?
                    <div className = "inputField" >
