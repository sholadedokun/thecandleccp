...
export default class multiplePost extends Component {
	constructor(props) {
		...
		this.state = {
			...
			collages: {
				2: [
					[{ x: "0|1", y: "0|2" }, { x: "1|1", y: "0|2" }],
					[{ x: "0|2", y: "0|1" }, { x: "0|2", y: "1|1" }],
					[{ x: "0|1.5", y: "0|2" }, { x: "1.5|0.5", y: "0|2" }],
					[{ x: "0|0.5", y: "0|2" }, { x: "0.5|1.5", y: "0|2" }]
				],
				3: [
					[{ x: "0|1", y: "0|2" }, { x: "1|1", y: "0|2" }, { x: "2|1", y: "0|2" }],
					[{ x: "0|3", y: "0|1" }, { x: "0|1.5", y: "1|1" }, { x: "1.5|1.5", y: "1|1" }],
					[{ x: "0|1.5", y: "0|1" }, { x: "1.5|1.5", y: "0|1" }, { x: "0|3", y: "1|1" }],
					[{ x: "0|2", y: "0|2" }, { x: "2|1", y: "0|1" }, { x: "2|1", y: "1|1" }],
					[{ x: "0|1", y: "0|1" }, { x: "0|1", y: "1|1" }, { x: "1|2", y: "0|2" }]
				]
			}
		};
	}
			...
			return (
				<div className={selectedIndex === index ? "collageContainer activeCollage" : "collageContainer"} onClick={this.setCollage.bind(this, index)} key={index} style={containerStyle}>
					{container.map((item, spIndex) => {
						...
						return <span key={spIndex} style={collageStyle} />;
					})}
				</div>
			);
		});
	...
	renderPostNo() {
		...
		return _.map(collages, (item, index) => {
			return (
				<li key={index} className={postNo == index ? "active" : ""} onClick={this.changeMediaNum.bind(this, index)}>
					{index}
				</li>
			);
		});
					...
					<p>
						...
						{(() => {
							switch (this.state.mediaTypes[index]) {
								case "image":
									return <input type="file" accept="image/*" onChange={this.props.renderPreview.bind(this, index, this.state.mediaTypes[index])} />;
								case "video":
									return <input type="file" accept="video/*" onChange={this.props.renderPreview.bind(this, index, this.state.mediaTypes[index])} />;
								case "url":
									return <input type="text" onBlur={this.props.renderPreview.bind(this, index, this.state.mediaTypes[index])} placeholder="Please type Web URL" />;
								default:
									return <input type="file" onChange={this.props.renderPreview.bind(this, index, this.state.mediaTypes[index])} />;
							}
						})()}
						<Heading size="xs" title="Upload Instruction" />
						<span>some Instruction about what to upload</span>
					</p>
				</Col>
			);
		});
