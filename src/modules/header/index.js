		...
		const navLinks = [
			{
				...
				component: <NavItem key='nav_item_1' className="userAccountCtrl">
				...
				displayOn: 'authenticated_bar_dashboard',
				component: <NavItem key='nav_item_2' onClick={() => this.props.modalStatus(true, modalRoute)}>
								<span className="actionButton post_campaign_ads">Post Ads</span>
							</NavItem>
			},{
				displayOn: 'authenticated_bar',
				component: <NavItem key='nav_item_3' onClick={()=>{this.goTo("/dashboard")}}>
								Dashboard
							</NavItem>
			},{
				displayOn: 'authenticated_bar',
				component: <NavItem key='nav_item_4' onClick={this.userLogin.bind(this, "signOut")}>
								<span className="buttonLink">Sign out</span>
							</NavItem>
			},{
				displayOn: 'authenticated_bar',
				component: <NavItem key='nav_item_5' onClick={() => this.props.modalStatus(true, modalRoute)}>
								<span className="buttonLink">Post Ads</span>
							</NavItem>
			},{
				displayOn: 'unAuthenticate',
				component: <NavItem key='nav_item_6' onClick={() => this.props.modalStatus(true, "login")} >
				...
				displayOn: 'unAuthenticate',
				component: <NavItem key='nav_item_7' onClick={() => this.props.modalStatus(true, "register")}>
								<span className="buttonLink">
									register
								</span>
							</NavItem>
				...
				component: <NavItem key='nav_item_8' onClick={()=>{this.goTo("/")}}>
								Feature
							</NavItem>
			},{
				displayOn: 'regularRoute',
				component: <NavItem key='nav_item_9'  onClick={()=>{this.goTo("/howitworks")}}>
								How it works
							</NavItem>
			},{
				displayOn: 'regularRoute',
				component: <NavItem key='nav_item_10' onClick={()=>{this.goTo("/help")}}>
								Spaces
							</NavItem>
			}
		];
		let resolvedLinks = navLinks.filter(link=>{
