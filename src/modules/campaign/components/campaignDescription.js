...
import AdvanceTiming from "../../../components/timingForm";
...
class AdSetDescription extends Component {
	...
	selectCampaign(newAllCampaigns, e) {
		console.log(e.target.value, newAllCampaigns);
		let campignName = newAllCampaigns.filter(item => item.id == e.target.value);
		this.setState({
			data: {
				...this.state.data,
				campaign_id: e.target.value,
				campaign_name: campignName[0].name,
				campaign_index: e.target.value
			}
		});
	}
						...
						<Row id="campaign_details" className="formSections">
							...
							<div className="inputField">
								...
								<span className={`formField rangeSelect ${errors.campaign_id ? "error" : ""}`}>
									<select onChange={this.selectCampaign.bind(this, newAllCampaigns)} value={campaign_id}>
